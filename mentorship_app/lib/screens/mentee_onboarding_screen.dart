...
class MenteeOnboardingScreen extends StatefulWidget {
  const MenteeOnboardingScreen({Key? key}) : super(key: key);
...
class _MenteeOnboardingScreenState extends State<MenteeOnboardingScreen> {
  ...
  Future<void> _saveAndContinue() async {
    ...
    try {
      ...
      if (uid == null) {
        throw Exception("currentUser.uid is null! User session lost before saving Mentee Goals.");
      }
      print('DEBUG: [Get Current User for Mentee Goals] Successful');

      await FirebaseFirestore.instance.collection('users').doc(uid).set({
        'goals': [text],
        'bio': text,
      }, SetOptions(merge: true)).timeout(const Duration(seconds: 10), onTimeout: () {
        throw Exception('Network timeout: Could not save goals. Please check your connection.');
      });
      print('DEBUG: [Save Mentee Goals] Successful');
      
      if (mounted) context.go('/onboarding/preferences');
    } catch (e, stackTrace) {
      print('ERROR: Failed at [Save Mentee Goals] - $e');
                ...
                child: GlassContainer(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'I am trying to transition from front-end to full-stack, but struggling with system design...',
                      hintStyle: TextStyle(
